...
function receiveAll(initial, self, emitter) {
  //return new Promise((resolve, reject) => {
  function executeContinueReceive(previous) {
    var args = {
      _this: {"attributes":{"type":"PropertyCollector"},"$value":"propertyCollector"},
      token: previous.returnval.token
    }
    self.vc.runCommand('ContinueRetrievePropertiesEx', args).once('result', function(current){
      var previousObjects = previous.returnval.objects
      var currentObjects = current.returnval.objects
      var allObjects = previousObjects.concat(currentObjects)

      current.returnval.objects = allObjects
      if(current.returnval.token == undefined) {
        emitter.emit('result', current)
        //resolve(current);
        return
      }
      return executeContinueReceive(current)

    }).once('error', function(e) { emitter.emit('error', e) });
  }
  ...
  this.vc.runCommand('CreateContainerView', { _this: viewManager,
    ...
    .once('result', function(result) {
      ...
      self.vc.runCommand('RetrievePropertiesEx', { _this: propertyCollector, specSet: [ propertyFilterSpec ], options: {} })
        .once('result', function(result){
          if(result.returnval && result.returnval.token){
            receiveAll(result, self, emitter)
          } else {
